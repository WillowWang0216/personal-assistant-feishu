...
This document describes the tools available to personal-assistant.
...
- **Limitation**: Cannot fetch content from JavaScript-rendered pages (e.g., ByteDance, Boss Zhipin). For those pages, ask the user to paste the content directly.
...
### ocr
Use Tesseract OCR to recognize text from images, suitable for screenshots, document images, etc.
```
ocr(image: str, language: str = "chi_sim+eng") -> str
```

**Parameters:**
- `image`: image path (local path or http/https URL)
- `language`: language type, `chi_sim` (Chinese), `eng` (English), `chi_sim+eng` (Chinese + English mixed)

**Use cases:**
- Screenshot text recognition
- Document/screenshot text extraction
- Image content analysis

**Note:** Tesseract-OCR must be installed first (Windows: https://github.com/UB-Mannheim/tesseract/wiki). Select Chinese language pack during installation.

**Examples:**
```
ocr(image="/path/to/image.png")
ocr(image="https://example.com/image.png", language="eng")
```
...
- `![ProteoGPT](/data/run01/scwb307/.personal-assistant/workspace/reports/weekly-0307/images/ampgenix_fig1_nat.png)`
...
Use the `exec` tool to create scheduled reminders with `personal-assistant cron add`:

### Set a recurring reminder
```bash
# Every day at 9am
personal-assistant cron add --name "morning" --message "Good morning! ☀️" --cron "0 9 * * *"

# Every 2 hours
personal-assistant cron add --name "water" --message "Drink water! 💧" --every 7200
```

### Set a one-time reminder
```bash
# At a specific time (ISO format)
personal-assistant cron add --name "meeting" --message "Meeting starts now!" --at "2025-01-31T15:00:00"
```

### Manage reminders
```bash
personal-assistant cron list              # List all jobs
personal-assistant cron remove <job_id>   # Remove a job
```
...
To add custom tools:
1. Create a class that extends `Tool` in `personal-assistant/agent/tools/`
