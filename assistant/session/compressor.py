...
from assistant.config.schema import ContextCompressionConfig
from assistant.providers.base import LLMProvider
from assistant.session.manager import Session
from assistant.utils.helpers import safe_filename, truncate_string
...
class SessionContextCompressor:
    ...
    async def _compress_locked(self, session: Session) -> bool:
        """Execute session compression under lock."""
    ...
    def _select_recent_indices(
        ...
        """Select message indices to keep with separate windows for dialog and tool-related messages."""
    ...
    def _active_messages(self, session: Session) -> list[tuple[int, dict[str, Any]]]:
        """Get list of active (context-included) messages with indices."""
    ...
    def _estimate_tokens(self, messages: list[tuple[int, dict[str, Any]]]) -> int:
        """Roughly estimate token count for a message list."""
        payload = []
    ...
    def _render_segment(self, old_segment: list[tuple[int, dict[str, Any]]]) -> str:
        """Render old message segment as readable text for LLM."""
        max_messages = 120
    ...
    def _trim_summary(self, summary: str) -> str:
        """Trim summary to maximum character limit."""
    ...
    def _save_summary_data(self, session_key: str, data: dict[str, Any]) -> None:
        """Save summary data to .summary.json file (atomic write)."""
        path = self._summary_path(session_key)
