...
from assistant.utils.helpers import ensure_dir, get_sessions_path, safe_filename, truncate_string
...
class Session:
    ...
    key: str  # Session identifier (channel:chat_id format)
...
class SessionManager:
    ...
    def _load_active_map(self) -> dict[str, str]:
        """Load channel:chat_id -> session_key mapping from _active.json."""
    ...
    def _save_active_map(self) -> None:
        """Persist active sessions mapping to _active.json."""
        try:
    ...
    def _load(self, key: str) -> Session | None:
        """Load a session from disk (JSONL format)."""
    ...
    def _infer_title_from_file(self, path: Path) -> str:
        """Infer title from the first user message in the file."""
        try:
            with open(path) as f:
                _ = f.readline()
                for _ in range(50):
