...
from assistant.utils.helpers import expand_path, get_data_path, get_workspace_path
...
class FeishuConfig(BaseModel):
    ...
    allow_from: list[str] = Field(default_factory=list)
    media_dir: str = Field(default_factory=lambda: str(get_data_path() / "media"))
    card_template_id: str = "AAqK6dMNHUVKE"
    card_template_version_name: str = "1.0.0"
    streaming_enabled: bool = False
    streaming_print_frequency_ms_default: int = 20
    streaming_print_step_default: int = 1
    streaming_print_strategy: Literal["fast", "delay"] = "delay"
    streaming_max_updates_per_sec: int = 50
    streaming_preemptive_timeout_sec: int = 480
    streaming_finalize_timeout_sec: int = 45
...
class IflytekConfig(BaseModel):
    """iFlytek file transcription tool configuration."""
    enabled: bool = True
    app_id: str = ""
    secret_key: str = ""
    language: str = "cn"
...
class CloudinaryConfig(BaseModel):
    ...
    def enabled(self) -> bool:
        """Whether the config is complete (cloud_name, api_key, api_secret all set)."""
...
class ToolsConfig(BaseModel):
    ...
    iflytek: IflytekConfig = Field(default_factory=IflytekConfig)
