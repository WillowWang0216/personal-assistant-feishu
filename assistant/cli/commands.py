"""CLI commands for personal-assistant."""
...
from assistant import __version__, __logo__

app = typer.Typer(
    name="personal-assistant",
    help=f"{__logo__} personal-assistant - Personal AI Assistant",
...
def version_callback(value: bool):
    """Version callback: prints logo and version, then exits."""
    if value:
        console.print(f"{__logo__} personal-assistant v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize personal-assistant configuration and workspace."""
    from assistant.config.loader import get_config_path, save_config
    from assistant.config.schema import Config
    from assistant.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} personal-assistant is ready!")
    console.print("\nNext steps:")
    console.print(f"  1. Add your API key to [cyan]{config_path}[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]personal-assistant agent -m \"Hello!\"[/cyan]")
...
I am personal-assistant, a lightweight AI assistant.
...
def gateway(
    ...
    """Start the personal-assistant gateway."""
    from assistant.config.loader import load_config, get_data_dir
    ...
    from assistant.cron.types import CronJob
    from assistant.heartbeat.service import HeartbeatService

    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)

    console.print(f"{__logo__} Starting personal-assistant gateway on port {port}...")
    ...
    if not api_key and not is_bedrock:
        ...
        console.print("Set one in your personal-assistant config file under providers.openrouter.apiKey or providers.openai.apiKey")
        raise typer.Exit(1)
    ...
    agent = AgentLoop(
        ...
        iflytek_config=config.tools.iflytek,
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from assistant.bus.events import OutboundMessage
...
def agent(
    ...
    if config.channels.feishu.enabled:
        try:
            from assistant.channels.feishu import FeishuChannel, FEISHU_AVAILABLE
            if FEISHU_AVAILABLE:
    ...
    if _cli_channels:
        from assistant.agent.tools.message import MessageTool as _MT
...
def memory_benchmark(
    ...
    history = [
        {"role": "user", "content": "We previously discussed long-term memory and prompt injection."},
        {"role": "assistant", "content": "OK, I will continue based on existing long-term memory."},
    ]
...
def _get_bridge_dir() -> Path:
    ...
    import shutil
    import subprocess
    from assistant.utils.helpers import get_bridge_path
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall personal-assistant")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from assistant.config.loader import get_data_dir
    from assistant.cron.service import CronService
...
def cron_add(
    ...
    from assistant.config.loader import get_data_dir
    from assistant.cron.service import CronService
    from assistant.cron.types import CronSchedule
...
@app.command()
def status():
    """Show personal-assistant status."""
    ...
    console.print(f"{__logo__} personal-assistant Status\n")
