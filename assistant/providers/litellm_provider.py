...
from assistant.providers.base import LLMProvider, LLMResponse, ToolCallRequest
...
class LiteLLMProvider(LLMProvider):
    ...
    Through unified interface calls OpenRouter, Anthropic, OpenAI, Gemini, and other backends.
    """
    ...
    def __init__(
        ...
        # Detect OpenRouter by api_key prefix or api_base URL
        self.is_openrouter = (
            (api_key and api_key.startswith("sk-or-")) or
            (api_base and "openrouter" in api_base)
        )

        # Detect vLLM endpoint (by model name or api_base)
        self.is_vllm = (
            bool(api_base)
        ...
        # Configure LiteLLM environment variables per provider
        if api_key:
    ...
    async def chat(
        ...
        Args:
            ...
            tools: Optional OpenAI-format tool definition list.
            model: Model identifier (e.g. 'anthropic/claude-sonnet-4-5').
            max_tokens: Maximum tokens in response.
            temperature: Sampling temperature.
            reasoning_effort: Reasoning effort parameter for reasoning models (optional).
        ...
        # OpenRouter: auto-add openrouter/ prefix
        if self.is_openrouter and not model.startswith("openrouter/"):
            model = f"openrouter/{model}"

        # Zhipu/Z.ai: ensure zai/ prefix
        ...
        # Moonshot/Kimi: ensure moonshot/ prefix
        ...
        # GPT-5 series rejects non-1.0 temperature in LiteLLM/OpenAI-compatible routing
        ...
        try:
            ...
            # Retry without tool_choice if some models don't support it
            ...
            # Fallback to raw HTTP for custom api_base
    ...
    def _should_retry_without_tool_choice(self, error: Exception) -> bool:
        """Determine if we should retry without tool_choice (some models don't support it)."""
    ...
    def _should_fallback_to_raw_http(self, error: Exception) -> bool:
        """Determine if we should fall back to raw HTTP for custom api_base."""
        message = str(error).lower()
        return (
    ...
    async def _chat_via_raw_openai_compatible_api(self, kwargs: dict[str, Any]) -> LLMResponse:
        """Send chat completion via raw HTTP for OpenAI-compatible endpoints."""
        payload = {
            "model": kwargs["model"],
    ...
    def _parse_openai_compatible_response(self, data: dict[str, Any]) -> LLMResponse:
        """Parse OpenAI-compatible format response."""
        ...
        for tc in message.get("tool_calls") or []:
            ...
            # arguments may be a JSON string, needs parsing
            if isinstance(args, str):
                try:
                    args = json.loads(args)
                except json.JSONDecodeError as e:
                    args = {
                        "__personal_assistant_tool_args_error__": "json_decode_error",
                        "__personal_assistant_tool_args_error_msg__": str(e),
                        "__personal_assistant_tool_args_raw__": args[:2000],
                    }
            if not isinstance(args, dict):
                args = {
                    "__personal_assistant_tool_args_error__": "non_object_arguments",
                    "__personal_assistant_tool_args_error_msg__": f"arguments must be a JSON object, got {type(args).__name__}",
                    "__personal_assistant_tool_args_raw__": str(args)[:2000],
                }
    ...
    def _get_usage_attr(usage: Any, key: str, default: Any = 0) -> Any:
        """Safely extract a field from usage object (dict or object attribute)."""
        if usage is None:
            return default
    ...
    def _extract_usage(self, usage: Any) -> dict[str, int]:
        """Extract and normalize token usage from response."""
        if not usage:
            return {}
        ...
        # Fallback: try multiple cache token field names from different providers
        if cache_tokens <= 0:
            for key in (
        ...
        # Some proxy providers expose a generic cache_tokens field, use as fallback capped at prompt_tokens
        if cache_tokens <= 0:
        ...
        # Some proxy providers underreport prompt_tokens, but total_tokens is accurate
        # If prompt_tokens < total_tokens - completion_tokens, use the derived value
