...
from assistant.bus.events import InboundMessage
...
from assistant.agent.tools.filesystem import AppendFileTool, EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
...
class SubagentManager:
    ...
    Subagents are lightweight instances that run in the background,
    sharing the LLM provider but with isolated context.
    """
    ...
    def __init__(
        ...
        from assistant.config.schema import ExecToolConfig, MineruConfig, NotionToolConfig, WebSearchConfig, ImageGenConfig, FeishuConfig
    ...
    def _accumulate_usage(cls, target: dict[str, int], usage: dict[str, Any] | None) -> None:
        """Accumulate token usage across LLM calls for the subagent lifecycle."""
    ...
    def _build_token_monitor(self, usage: dict[str, int]) -> dict[str, Any]:
        ...
        # Derive input tokens (for cache token limits)
    ...
    async def spawn(
        ...
        Args:
            ...
            label: Readable label for display in logs and notifications.
            origin_channel: Target channel for result notifications.
            origin_chat_id: Target chat ID for result notifications.
    ...
    async def _run_subagent(
        ...
        try:
            # Build tool set (excluding spawn to prevent subagent loops)
            tools = ToolRegistry()
            ...
            # Build messages (including subagent-specific system prompt)
            ...
            # Run agent loop (with iteration limit)
            max_iterations = 100
            iteration = 0
            ...
            await self._announce_result(
                ...
                {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0, "cache_tokens": 0},
            )
...
Summarize this naturally for the user. Keep it brief (1-2 sentences). Do not mention technical details like "subagent" or task IDs."""
        ...
        # Inject as system message to trigger main agent processing
        msg = InboundMessage(
            channel="system",
            sender_id="subagent",
