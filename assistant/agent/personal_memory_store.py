...
from assistant.utils.helpers import ensure_dir


# Memory type weights for relevance scoring
KIND_WEIGHTS = {
    "constraint": 1.4,
    "preference": 1.3,
    "decision": 1.2,
...
class PersonalMemoryStore:
    ...
    def _row_to_memory(self, row: sqlite3.Row | None) -> dict[str, Any] | None:
        """Convert sqlite3.Row to memory dict."""
        if row is None:
            return None
        data = dict(row)
    ...
    def _upsert_fts(self, conn: sqlite3.Connection, memory_id: str, summary: str, content: str, tags: list[str], keywords: list[str]) -> None:
        """Update FTS5 full-text search index for a memory."""
    ...
    def add_candidates(self, candidates: list[dict[str, Any]], extracted_from: str, user_id: str | None = None) -> list[str]:
        """Add LLM-extracted candidate memories to the database."""
        if not candidates:
            return []
    ...
    def get_unmerged_candidates(self, limit: int = 100, user_id: str | None = None) -> list[dict[str, Any]]:
        """Get all unmerged candidate memories."""
        user_id = user_id or self.config.default_user_id
        with self._connect() as conn:
            rows = conn.execute(
    ...
    def find_related_memories(self, candidate: dict[str, Any], limit: int = 5, user_id: str | None = None) -> list[dict[str, Any]]:
        """Find existing memories related to a candidate."""
        user_id = user_id or self.config.default_user_id
    ...
    def create_memory(self, memory: dict[str, Any], user_id: str | None = None) -> str:
        """Create a new memory and insert into database + FTS index."""
        user_id = user_id or self.config.default_user_id
    ...
    def update_memory(self, memory_id: str, new_memory: dict[str, Any]) -> None:
        """Update an existing memory (merging tags, keywords, etc.)."""
        current = self.get_memory(memory_id)
        if not current:
            raise ValueError(f"Memory not found: {memory_id}")
        tags = list(dict.fromkeys((current.get("tags") or []) + (new_memory.get("tags") or [])))
        keywords = list(dict.fromkeys((current.get("keywords") or []) + (new_memory.get("keywords", []))))
        source_refs = list(dict.fromkeys((current.get("source_refs") or []) + (new_memory.get("source_refs", []))))
        with self._connect() as conn:
            conn.execute(
                """
                UPDATE memories SET
    ...
    def supersede_memory(self, old_id: str, new_memory: dict[str, Any], user_id: str | None = None) -> str:
        """Supersede an old memory and create a replacement."""
        user_id = user_id or self.config.default_user_id
        with self._connect() as conn:
    ...
    def list_core_candidates(self, user_id: str | None = None, limit: int | None = None) -> list[dict[str, Any]]:
        """List core memory candidates (scope=global, ordered by priority and recency)."""
        user_id = user_id or self.config.default_user_id
    ...
    def list_active_memories(self, user_id: str | None = None, limit: int = 500) -> list[dict[str, Any]]:
        """List all active memories (status=active)."""
        user_id = user_id or self.config.default_user_id
        with self._connect() as conn:
            rows = conn.execute(
    ...
    def _ensure_idf_ready(self) -> None:
        """Precompute BM25 IDF values from all active memories."""
        all_memories = self.list_active_memories(limit=5000)
        if not all_memories:
            return

        N = len(all_memories)
        if N == self._bm25_n and self._bm25_idf:
            return
        ...
        for memory in all_memories:
            ...
            tokens = set(self._tokenize(summary) + self._tokenize(content))
            for t in tokens:
                token_doc_freq[t] = token_doc_freq.get(t, 0) + 1
            total_doc_len += len(summary) + len(content)

        self._bm25_idf = {}
        for t, df in token_doc_freq.items():
            self._bm25_idf[t] = math.log((N - df + 0.5) / (df + 0.5) + 1)

        self._bm25_n = N
        self._bm25_avgdl = total_doc_len / N if N > 0 else 1.0

    def _bm25_score(self, q_tokens: list[str], summary_tokens: list[str], content_tokens: list[str], doc_len: int) -> float:
        """Calculate BM25 score for a single memory."""
        if not q_tokens:
            return 0.0

        all_doc_tokens = summary_tokens + content_tokens
        score = 0.0
        avgdl = max(self._bm25_avgdl, 1.0)

        for t in q_tokens:
            tf = all_doc_tokens.count(t)
            if tf == 0:
                continue
            idf = self._bm25_idf.get(t, math.log((self._bm25_n + 1) / 2 + 1))
            tf_component = (tf * (self._bm25_k1 + 1)) / (tf + self._bm25_k1 * (1 - self._bm25_b + self._bm25_b * doc_len / avgdl))
    ...
    def retrieve(
        ...
        """Hybrid scoring retrieval: BM25 + tag + keyword + priority + scope + recency + kind weight."""
        user_id = user_id or self.config.default_user_id
        ...
        for memory in all_memories:
            ...
            bm25_score_val = self._bm25_score(q_tokens, summary_tokens, content_tokens, doc_len)
            ...
            score = (
                weights.keyword * bm25_score_val
    ...
    def mark_used(self, memory_ids: list[str]) -> None:
        """Mark memories as used (update last_used_at timestamp)."""
        if not memory_ids:
            return
        now = self._now_iso()
        with self._connect() as conn:
    ...
    def _tokenize(self, text: str) -> list[str]:
        """Tokenize using jieba exact mode with English/symbol handling."""
        if not text:
            return []
        tokens = list(jieba.cut(text, cut_all=False))
        result = list(dict.fromkeys([t.strip() for t in tokens if t.strip()]))[:64]
        return result

    def sync_memory_md(self, extra_notes: list[str] | None = None, user_id: str | None = None) -> None:
        """Sync MEMORY.md index with the database."""
