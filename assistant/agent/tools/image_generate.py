...
from assistant.config.schema import FeishuConfig, ImageGenConfig
...
class ImageGenerateTool(Tool):
	...
	def _build_prompt(self, prompt: str, aspect_ratio: str) -> str:
		"""Build full prompt with aspect ratio requirement appended."""
		suffix = ""
	...
	async def _generate_image(
		...
		"""Call image generation API with multi-image input and auto-retry."""
		if "dashscope" in (self.config.api_base or ""):
			return await self._generate_wanxiang(full_prompt, images)
	...
	async def _generate_wanxiang(
		...
		"""Wanxiang image generation: returns URL, needs download."""
		url = urljoin(self.config.api_base.rstrip("/") + "/", "generation")
		user_content: list[dict[str, Any]] = [{"type": "text", "text": full_prompt}]
		if images:
			user_content.extend(images)
		...
		async with httpx.AsyncClient(timeout=self.config.timeout) as client:
			for attempt in range(1, max_attempts + 1):
				try:
					...
					last_error = e
					if attempt >= max_attempts:
						raise RuntimeError(f"request failed after {max_attempts} attempts: {e}") from e
					if backoff > 0:
						await asyncio.sleep(backoff)
						backoff = min(self.config.retry_max_backoff_seconds, backoff * self.config.retry_backoff_multiplier)
					continue
				...
				try:
					...
					last_error = e
					if e.response.status_code in retry_status_codes and attempt < max_attempts:
						if backoff > 0:
							await asyncio.sleep(backoff)
							backoff = min(self.config.retry_max_backoff_seconds, backoff * self.config.retry_backoff_multiplier)
						continue
		...
		if not image_url:
			raise RuntimeError(f"no image URL found in Wanxiang response: {str(data)[:200]}")

		# Download image bytes from URL
		img_bytes = await self._download_image_from_url(image_url)
		mime_type = _guess_mime_from_url(image_url)
		return img_bytes, mime_type
	...
	async def _generate_image_original(
		self,
		full_prompt: str,
		images: list[dict[str, Any]] | None = None,
	) -> tuple[bytes, str]:
		"""Call image generation API with multi-image input and auto-retry (OpenAI compatible format)."""
		url = urljoin(self.config.api_base.rstrip("/") + "/", "chat/completions")
		user_content: list[dict[str, Any]] = [{"type": "text", "text": full_prompt}]
		if images:
			user_content.extend(images)
	...
	def _save_image(self, img_bytes: bytes, mime_type: str, output_path: str | None) -> str:
		...
		if output_path:
			...
			# Default save location: workspace/outputs/images/
	...
	def _collect_images(
		...
		"""Collect all local images to pass, converting to base64 format."""
		paths: list[str] = []
		if image_path:
	...
	def _resolve_aspect_ratio(self, aspect_ratio: str, images: list[dict[str, Any]]) -> str:
		"""Default to original aspect ratio when reference images are present."""
	...
	def _send_feishu_image_post(self, image_key: str, chat_id: str, title: str) -> None:
		"""Send image via Feishu message card."""
		if not FEISHU_AVAILABLE:
			raise RuntimeError("Feishu SDK not installed")
		if not self.feishu_config.app_id or not self.feishu_config.app_secret:
			raise RuntimeError("Feishu app_id/app_secret not configured")
		...
		# Build rich text message content (Feishu post type)
		post_content = {
			"zh_cn": {
				"title": title,
...
def _resolve_path(path: str, allowed_dir: Path | None = None) -> Path:
	"""Resolve path and check if within allowed directory scope."""
...
def _extract_content(payload: dict[str, Any]) -> str | list[dict[str, Any]]:
	"""Extract content field from API response payload."""
	try:
...
def _extract_image_from_payload(payload: dict[str, Any]) -> tuple[str, str] | None:
	"""Extract base64 image data from API response payload."""
	try:
		choices = payload.get("choices") or []
		if choices:
			message = choices[0].get("message") or {}
...
def _describe_payload(payload: dict[str, Any]) -> str:
	"""Generate human-readable payload description for error messages."""
	try:
		choices = payload.get("choices") or []
		if choices:
			message = choices[0].get("message") or {}
...
def _extract_image_data(text: str) -> tuple[str, str] | None:
	"""Extract data URI or Markdown format image from text."""
	if not text:
		return None

	# Prefer matching data:image/...;base64,... format
	...
	# Fallback: Markdown image format ![image](data:image/...)
