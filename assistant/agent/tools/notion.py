...
from assistant.config.schema import NotionToolConfig
...
_NOTION_CODE_LANGUAGE_ALIASES = {
    ...
    "console": "shell", "shellscript": "shell", "sh": "shell", "zsh": "shell",
    "bash script": "bash",
    "js": "javascript", "ts": "typescript", "py": "python",
    "yml": "yaml", "md": "markdown",
    "objc": "objective-c", "objective c": "objective-c", "objective_c": "objective-c",
    "csharp": "c#", "cs": "c#", "cpp": "c++", "cxx": "c++",
    "golang": "go",
}
...
class NotionTool(Tool):
    ...
    async def _inspect_database(self, doc_type: str = "auto") -> str:
        """Inspect database schema and sample content."""
        ...
        return json.dumps({"status": "ok", "action": "inspect_database", "targets": inspected}, ensure_ascii=False, indent=2)

    async def _ensure_partitions(self) -> str:
        """Ensure partition labels (reports/notes/log etc.) exist in database."""
        ...
        for alias, database_id in targets.items():
            ...
            if not prop:
                results.append(f"[{alias}] Type property '{type_name}' not found (skip).")
                continue
            ...
            if prop_type == "select":
                ...
                results.append(f"[{alias}] ensured in select '{type_name}': {', '.join(required_labels)}")
                continue
            ...
            if prop_type == "multi_select":
                ...
                results.append(f"[{alias}] ensured in multi_select '{type_name}': {', '.join(required_labels)}")
                continue

            results.append(f"[{alias}] property '{type_name}' type '{prop_type}' unsupported; use select/multi_select/rich_text")
    ...
    async def _upload_file(
        ...
        """Upload local file to Notion database."""
        ...
        # Build child blocks (render Markdown/code blocks)
        ...
        res = {
            "status": "ok", "action": "upload_file", "page_id": page_id,
            "url": created.get("url"), "file": source_path, "doc_type": actual_type,
            "title": page_title, "database_id": database_id,
    ...
    async def _list_items(self, doc_type: str = "auto", limit: int = 10) -> str:
        ...
        if doc_type and doc_type.strip().lower() != "auto":
            ...
            data = await self._request("POST", f"databases/{database_id}/query", json_body=query_body)

            items = [self._page_brief(p) for p in data.get("results", [])]
            return json.dumps({"status": "ok", "action": "list_items", "count": len(items), "database_id": database_id, "items": items}, ensure_ascii=False, indent=2)
        ...
        for alias, database_id in targets.items():
            data = await self._request("POST", f"databases/{database_id}/query", json_body={"page_size": limit})
            items = [self._page_brief(p) for p in data.get("results", [])]
            all_items.extend(items)
            per_target.append({"alias": alias, "database_id": database_id, "count": len(items), "items": items})

        return json.dumps({"status": "ok", "action": "list_items", "count": len(all_items), "targets": per_target}, ensure_ascii=False, indent=2)

    async def _reclassify_item(self, page_id: str, doc_type: str) -> str:
        """Reclassify a Notion entry by updating its type property."""
        ...
        if parent_db and parent_db != target_database_id:
            return "Error: reclassify across different databases is not supported by Notion page update. Use upload_file to create a new page in target database, then archive the original page manually."
        ...
        if not patch_properties:
            return f"Error: cannot set type. Property '{self.config.type_property}' missing or unsupported (supported: select/multi_select/rich_text)."

        updated = await self._request("PATCH", f"pages/{self._normalize_page_id(page_id)}", json_body={"properties": patch_properties})
        return json.dumps({"status": "ok", "action": "reclassify_item", "page_id": updated.get("id"), "doc_type": target_type, "database_id": target_database_id, "url": updated.get("url")}, ensure_ascii=False, indent=2)

    def _read_text_content(self, file_path: Path) -> str:
        """Read text-extractable file content."""
    ...
    def _build_children(self, file_path: Path, doc_type: str, full_text: str) -> list[dict[str, Any]]:
        """Build Notion blocks based on file type."""
        info = (
            f"Ingested by personal-assistant notion tool\\n"
            f"File: {file_path.name}\\n"
            f"Path: {file_path}\\n"
            f"Partition: {doc_type}"
        )
        children = [{"object": "block", "type": "paragraph", "paragraph": {"rich_text": self._inline_to_rich_text(info)}}]
    ...
    async def _append_children(self, page_id: str, children: list[dict[str, Any]]) -> None:
        """Append child blocks in chunks (Notion API max 80 per request)."""
    ...
    def _markdown_to_blocks(self, markdown_text: str, base_dir: Path | None = None) -> list[dict[str, Any]]:
        """Convert Markdown text to Notion blocks list."""
        ...
        while i < len(lines):
            ...
            # Inline block equation: $$ expression $$
    ...
    def _flush_paragraph_buffer(self, buffer: list[str], blocks: list[dict[str, Any]]) -> None:
        """Flush paragraph buffer lines as a single paragraph block."""
        if not buffer:
            return
        text = "\n".join(buffer).strip()
        if text:
    ...
    def _parse_list_item(line: str):
        """Parse a list item, returning (indent, type, text, checked) or None."""
    ...
    def _make_list_block(self, item_type: str, text: str, checked=None, children: list | None = None) -> dict[str, Any]:
        """Create a single list block (optionally with children)."""
        ...
        return {"object": "block", "type": item_type, item_type: body}

    def _build_nested_list_blocks(self, run_lines: list[str]) -> list[dict[str, Any]]:
        """Parse consecutive list lines into nested Notion blocks."""
    ...
    def _text_blocks(self, block_type: str, text: str) -> list[dict[str, Any]]:
        """Convert text to specified block type."""
        rich_text = self._inline_to_rich_text(text)
        if not rich_text:
            rich_text = self._inline_to_rich_text(" ")
    ...
    def _code_blocks_from_text(self, text: str, language: str = "plain text") -> list[dict[str, Any]]:
        ...
        for chunk in self._split_chunks(text, _MAX_RICH_TEXT_CHARS):
            blocks.append({
                ...
                "code": {"language": notion_language, "rich_text": [{"type": "text", "text": {"content": chunk}}]},
            })
        return blocks
    ...
    def _inline_to_rich_text(self, text: str) -> list[dict[str, Any]]:
        ...
        for match in _INLINE_PATTERN.finditer(text):
            ...
            if match.group("math") is not None:
                ...
                if expression:
                    rich_text.append({"type": "equation", "equation": {"expression": expression}, "annotations": self._default_annotations()})
    ...
    def _apply_annotation_to_nodes(self, nodes: list[dict[str, Any]], extra: dict[str, Any]) -> list[dict[str, Any]]:
        """Merge annotation flags onto rich_text nodes."""
        result: list[dict[str, Any]] = []
        for node in nodes:
            if node.get("type") == "equation":
                result.append(node)
                continue
            new_node = {"type": node["type"], "text": dict(node["text"])}
    ...
    def _default_annotations(self, annotations: dict[str, Any] | None = None) -> dict[str, Any]:
        """Get default annotations with optional overrides."""
        merged = {"bold": False, "italic": False, "strikethrough": False, "underline": False, "code": False, "color": "default"}
        if annotations:
    ...
    def _append_text_segment(self, rich_text: list[dict[str, Any]], content: str, annotations: dict[str, Any], link_url: str | None = None) -> None:
        """Append a text segment to rich_text (auto-chunked by max char limit)."""
        if not content:
            return
    ...
    def _build_table_block(self, lines: list[str]) -> dict[str, Any] | None:
        """Parse Markdown table rows into a Notion table block."""
        rows: list[list[str]] = []
        for raw in lines:
            row = raw.strip()
            if not row:
        ...
        for row in rows:
            ...
            normalized_rows.append({
                ...
                "table_row": {"cells": [self._inline_to_rich_text(cell or " ") for cell in normalized]},
            })

        return {"object": "block", "type": "table", "table": {"table_width": width, "has_column_header": has_column_header, "has_row_header": False, "children": normalized_rows}}

    def _is_alignment_row(self, cells: list[str]) -> bool:
        """Check if a row is a Markdown table alignment row (e.g. |:---|:---|)."""
        if not cells:
            return False
        return all(bool(re.match(r"^:?-{3,}:?$", c.strip())) for c in cells)

    def _is_table_line(self, line: str) -> bool:
        """Quick check if a line looks like a Markdown table row."""
        if "|" not in line:
            return False
    ...
    def _split_table_cells(self, line: str) -> list[str]:
        """Smart-split table cells (handles escapes, backtick wrapping, and bracket content)."""
        row = line.strip()
        if row.startswith("|"):
            row = row[1:]
        if row.endswith("|"):
    ...
    def _is_valid_notion_link_url(self, url: str) -> bool:
        """Check if URL is a valid Notion link (needs http(s) scheme and domain)."""
        if not url:
            return False
        parsed = urlparse(url)
    ...
    def _parse_code_fence_open(self, stripped_line: str) -> tuple[str, str] | None:
        """Parse Markdown code fence opening."""
    ...
    def _is_code_fence_close(self, stripped_line: str, opening_fence: str) -> bool:
        """Check if a line is the matching code fence close."""
    ...
    def _to_notion_code_language(self, language: str | None) -> str:
        """Convert language identifier to a Notion-accepted enum value."""
    ...
    def _build_type_filter(self, schema: dict[str, Any], doc_type: str) -> dict[str, Any] | None:
        """Build Notion query filter by doc_type."""
    ...
    def _set_property_value(self, schema: dict[str, Any], container: dict[str, Any], property_name: str, value: str) -> None:
        """Set a property value on a page properties container based on property type."""
        ...
        if prop_type == "rich_text":
            container[property_name] = {"rich_text": [{"type": "text", "text": {"content": text_value}}]}
            return
    ...
    def _normalize_doc_type(self, doc_type: str, file_path: Path | None) -> str:
        """Normalize document type (auto mode infers from filename)."""
        ...
        if configured_map:
            if "log" in configured_map and any(kw in source for kw in ["log", "日志", "trace"]):
                return "log"
            if "reports" in configured_map and any(kw in source for kw in ["report", "日报", "周报", "月报", "总结", "analysis", "分析"]):
                return "reports"
    ...
    def _resolve_target_database_id(self, doc_type: str) -> str:
        """Resolve target database ID by doc_type."""
    ...
    def _get_target_databases(self, doc_type: str = "auto") -> dict[str, str]:
        """Get target database mapping (deduplicated)."""
    ...
    def _find_title_property(self, schema: dict[str, Any]) -> str | None:
        """Find the title property of a database."""
    ...
    def _page_brief(self, page: dict[str, Any]) -> dict[str, Any]:
        """Extract a brief summary of a page."""
        title = "Untitled"
    ...
    def _extract_type(self, properties: dict[str, Any]) -> str:
        """Extract document type from page properties."""
    ...
    def _normalize_page_id(self, page_id: str) -> str:
        """Normalize Notion page ID format (32-char raw -> standard dashed format)."""
    ...
    def _resolve_path(self, path: str) -> Path:
        """Resolve file path and check permissions."""
    ...
    def _resolve_image_url(self, src: str, base_dir: Path | None = None) -> str | None:
        """Resolve image URL: HTTP(s) returns as-is, local paths uploaded to Cloudinary."""
    ...
    def _image_block(self, url: str, caption: str = "") -> dict[str, Any]:
        ...
        if caption:
            block["image"]["caption"] = [{"type": "text", "text": {"content": caption[:_MAX_RICH_TEXT_CHARS]}}]
        return block
