...
from assistant.utils.helpers import truncate_string
...
class SessionManageTool(Tool):
    ...
    def _create_session(
        ...
        Key format: channel:chat_id[:slug] or channel:chat_id:timestamp.
        """
    ...
    def _list_sessions(self, limit: int) -> str:
        """List all sessions, marking the currently active one with *."""
    ...
    def _reset_session(self) -> str:
        """Clear active session override, reverting to default channel session."""
        if not self._channel or not self._chat_id:
            return "Error: no session context (channel/chat_id)"
    ...
    def _resolve_key_for_create(self, session_key: str | None) -> str:
        """
        Resolve key for create operation.

        Format: channel:chat_id[:slug] or channel:chat_id:timestamp.
        """
    ...
    def _resolve_key_for_switch(self, session_key: str) -> str | None:
        """Resolve key for switch operation, trying multiple matching strategies."""
    ...
    def _generate_title(self, seed: str | None) -> str:
        """Auto-generate a title from seed text."""
        if seed:
